...
const populateThreads = async (ctx: QueryCtx, messageId: Id<"messages">) => {
    ...
    const lastMessageMember = await populateMember(ctx, latestMessage.memberId);
...
export const get = query({
    ...
    handler: async (ctx, args) => {
        ...
        const results = await ctx.db
            .query("messages")
            .withIndex("by_channel_id_parent_conversation_id", (q) =>
                q
            ...
            page: (
                await Promise.all(
                    results.page.map(async (message) => {
                        const member = await populateMember(ctx, message.memberId);
                        const user = member
                            ? await populateUser(ctx, member.userId) : null;
